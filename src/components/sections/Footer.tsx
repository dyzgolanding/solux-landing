import { Mail, Phone, MapPin } from 'lucide-react'
import { Container } from '@/components/ui/Container'
import Image from 'next/image'

const navLinks = [
  { label: '¿Cómo funciona?', href: '#como-funciona' },
  { label: 'Beneficios', href: '#beneficios' },
  { label: 'Proceso', href: '#proceso' },
  { label: 'Testimonios', href: '#testimonios' },
  { label: 'Preguntas frecuentes', href: '#faq' },
]

const SocialIcons = {
  Instagram: () => (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
    </svg>
  ),
  Facebook: () => (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  ),
  LinkedIn: () => (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  ),
}

const social = [
  { Icon: SocialIcons.Instagram, href: 'https://www.instagram.com/soluxenergy.cl', label: 'Instagram' },
]

export function Footer() {
  return (
    <footer className="bg-[#060E1A] text-white">
      {/* Top section */}
      <div className="border-b border-white/8">
        <Container className="py-10 lg:py-14">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-10">
            {/* Brand column */}
            <div className="lg:col-span-2">
              {/* Logo */}
              <div className="mb-5">
                <Image
                  src="/favicon.ico"
                  alt="SOLUX ENERGY"
                  width={64}
                  height={64}
                  className="h-10 w-auto object-contain"
                  unoptimized
                />
              </div>

              <p className="text-white/50 text-sm leading-relaxed mb-6 max-w-xs">
                Empresa chilena especializada en energía solar residencial. Productos premium, evaluación personalizada y acompañamiento en todo el proceso: desde la cotización hasta la puesta en marcha.
              </p>

              {/* Contact */}
              <div className="flex flex-col gap-3 text-sm text-white/50">
                <a href="mailto:contacto@soluxenergy.cl" className="flex items-center gap-2 hover:text-white transition-colors duration-200">
                  <Mail className="w-4 h-4 text-[#0A6EBD]" />
                  contacto@soluxenergy.cl
                </a>
                <a href="tel:+56934015468" className="flex items-center gap-2 hover:text-white transition-colors duration-200">
                  <Phone className="w-4 h-4 text-[#22C55E]" />
                  +56 9 3401 5468
                </a>
                <span className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-[#F5A623]" />
                  Santiago, Chile · Cobertura nacional
                </span>
              </div>

              {/* Social */}
              <div className="flex gap-3 mt-6">
                {social.map((s) => {
                  const { Icon } = s
                  return (
                    <a
                      key={s.label}
                      href={s.href}
                      aria-label={s.label}
                      className="w-9 h-9 rounded-xl bg-white/8 hover:bg-[#0A6EBD] flex items-center justify-center transition-all duration-200 hover:scale-110"
                    >
                      <Icon />
                    </a>
                  )
                })}
              </div>
            </div>

            {/* Navegación */}
            <div>
              <h4 className="text-white font-bold text-sm uppercase tracking-wider mb-4">Navegación</h4>
              <ul className="flex flex-col gap-2.5">
                {navLinks.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="text-white/45 hover:text-white text-sm transition-colors duration-200">
                      {link.label}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* CTA block */}
            <div className="lg:col-span-2">
              <h4 className="text-white font-bold text-sm uppercase tracking-wider mb-4">Empieza hoy</h4>
              <p className="text-white/45 text-sm leading-relaxed mb-5">
                Cotización gratuita y sin compromiso. Te respondemos el mismo día.
              </p>
              <div className="flex flex-col gap-3">
                <a
                  href="https://cotiza.soluxenergy.cl"
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-[#F5A623] to-[#FBBF24] text-[#0A1628] font-bold text-sm hover:scale-105 transition-transform duration-200"
                >
                  Cotiza gratis →
                </a>
                <a
                  href="https://wa.me/56934015468?text=Hola%20SOLUX%20Energy%2C%20quiero%20información%20sobre%20paneles%20solares"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl border border-white/15 text-white/70 font-semibold text-sm hover:bg-white/8 hover:text-white transition-all duration-200"
                >
                  <Phone className="w-4 h-4 text-[#22C55E]" />
                  WhatsApp directo
                </a>
              </div>
            </div>
          </div>
        </Container>
      </div>

      {/* Bottom bar */}
      <Container className="py-6">
        <div className="flex items-center justify-center text-xs text-white/30">
          <span>© {new Date().getFullYear()} SOLUX ENERGY — Todos los derechos reservados</span>
        </div>
      </Container>
    </footer>
  )
}
